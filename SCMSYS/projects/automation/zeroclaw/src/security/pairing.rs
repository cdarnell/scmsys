...
mod tests {
    ...
    #[test]
    async fn pairing_code_consumed_and_token_persisted() {
        let guard = PairingGuard::new(true, &[]);
        let code = guard
            .pairing_code()
            .expect("pairing code should exist before first client pairs");

        let token = guard
            .try_pair(&code, "test_client")
            .await
            .expect("pairing attempt should succeed")
            .expect("token should be issued on success");

        // Successful pairing should clear the one-time code and record a hashed token.
        assert!(guard.pairing_code().is_none());

        let stored_tokens = guard.tokens();
        assert_eq!(stored_tokens.len(), 1);
        assert_ne!(stored_tokens[0], token, "gateway state must store hashed tokens, not plaintext");
